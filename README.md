...
`tools/list` at `https://gateway.pipeworx.io/opentopography/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/point_elevation \
  -H 'Content-Type: application/json' \
  -d '{"lat":40.7128,"lon":-74.006,"dataset":"SRTMGL3"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/point_elevation`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
